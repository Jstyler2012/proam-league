...
exports.handler = async (event) => {
  try {
    ...
    // helper: Supabase REST calls (anon)
    ...
    // CURRENT WEEK (returns full scheduled week fields)
    ...
    // SEASON STANDINGS (points desc)
    // Backed by your view public.season_standings
    if (path === "season-standings") {
      ...
      return {
        statusCode: 200,
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ rows: out.json || [] }),
      };
    }

    // DEBUG (optional)
    if (path === "debug-week-entries") {
      const out = await sb("GET", "week_entries?select=*&limit=1");
      if (!out.ok) return { statusCode: out.status, body: out.text };
      return {
        statusCode: 200,
        headers: { "content-type": "application/json" },
    ...
    if (path === "leaderboard") {
      ...
      // 4) Sort: combined asc, nulls last; tie-breaker name
      rows.sort((a, b) => {
        const ac = a.combined;
        const bc = b.combined;
