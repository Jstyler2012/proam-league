console.log("SYNC-FIELD-MANUAL LOADED");
...
exports.handler = async (event) => {
  try {
    ...
    if (!Number.isFinite(week_number)) return json(400, { ok: false, error: "Missing/invalid week_id" });
    ...
    // Simple probe read (we’ll add insert logic once we confirm connectivity)
    ...
    return json(200, {
      ...
      message: "Manual HTTP function works. Now we can implement roster upsert into week_pro_field.",
    });
  } catch (err) {
    console.error("sync-field-manual error:", err);
